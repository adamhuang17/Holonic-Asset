...
- 浏览器只访问 Caddy；业务 HTTP 请求进入 Core API。
...
Docker Compose 部署 Frontend、Caddy、Core API、AI Service、Asset Worker、PostgreSQL、Redis、NATS JetStream 和 S3 兼容对象存储。后续是否引入更重的编排平台另行决策。
